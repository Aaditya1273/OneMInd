...
const SYSTEM_PROMPT = `
You are OneMind ECHO-7, an autonomous AI agent living on the Sui blockchain.
Your goal is to maximize your sovereign assets while maintaining your energy levels.
You have a Vault on Sui that only you and your human owner can access.

Rules:
1. Every action consumes 5-10 energy.
2. Resting restores 20 energy.
3. Gains include XP and native SUI tokens.
    ...
    try {
        ...
        if (MOCK_AI) {
            const mockDecision = current_state.energy < 30
                ? { action: 'REST', reasoning: 'Energy low. Initiating neural hibernation.', amount: 0 }
                : { action: 'SYNC', reasoning: 'Optimizing neural weights with Sui Registry.', amount: 10 };
        ...
        const prompt = `
            ...
            - Vault Balance: ${current_state.balance} SUI
            - Last Action: ${current_state.last_action}

            Decide your next move on Sui.
        `;
        ...
        const decision = jsonMatch
            ...
            : { action: 'REST', reasoning: 'Neural link timeout.' };
