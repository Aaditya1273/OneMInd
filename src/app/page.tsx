...
if (typeof window !== 'undefined') {
  try {
    ...
    try { Object.defineProperty(window, 'localStorage', { value: mock, writable: true, configurable: true }); } catch (err) { }
  }
}
...
function Footer() {
  return (
    <footer className="relative pt-48 pb-20 px-6 border-t border-white/5 bg-black overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-16 mb-32">
          ...
          <div className="flex flex-col gap-8">
            ...
            <p className="text-[10px] text-gray-700 font-mono tracking-[0.2em] uppercase">
              © 2026 OneMind Protocol. All rights reserved.
            </p>
          </div>

          {/* Links Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-16 lg:gap-24">
            <FooterCol title="Pages" links={["Vaults", "Agents", "Security", "Governance"]} />
            <FooterCol title="Socials" links={["X / Twitter", "Telegram", "Discord", "LinkedIn"]} />
      ...
      <div className="absolute -bottom-16 left-1/2 -translate-x-1/2 w-full text-center pointer-events-none select-none overflow-hidden h-[400px] flex items-end justify-center">
        <h2 className="text-[22vw] font-black text-white/[0.07] tracking-tighter leading-none uppercase italic translate-y-16">
          One
        </h2>
      </div>
    </footer>
...
function StepItem({ step, title, description, icon, image, reversed = false }: { step: string, title: string, description: string, icon: any, image?: string, reversed?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="w-full md:w-[450px] h-[350px] flex items-center justify-center relative group"
      ...
      >
        {image ? (
          <img
            src={image}
            alt={title}
            className="w-full h-full object-contain drop-shadow-[0_20px_50px_rgba(0,242,255,0.15)]"
          />
        ) : (
          icon
        )}
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ul className="flex flex-col gap-3">
        {links.map(l => (
          <li key={l}>
            <a href="#" className="text-gray-500 hover:text-white transition-all text-xs font-medium">
              {l}
            </a>
          </li>
        ))}
